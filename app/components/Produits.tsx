    ...
    <section id="produits" style={{height: '100vh', position: 'relative', display: 'flex', flexDirection: 'column', overflow: 'hidden'}}>
      ...
      <div style={{position: 'relative', zIndex: 2, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', height: '100%', padding: isMobile ? '72px 16px 24px' : '56px 80px 30px', gap: isMobile ? '8px' : '12px'}}>
        ...
        <div key={`text-${index}`} style={{textAlign: 'center', flexShrink: 0, animation: 'fadeIn 0.4s ease', padding: isMobile ? '0 8px' : '0'}}>
          ...
          <p style={{fontSize: '12px', color: 'rgba(245,235,224,0.6)', lineHeight: 1.6, maxWidth: '480px', margin: '0 auto 12px'}}>{p.desc}</p>
          <div style={{display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '16px'}}>
            <button onClick={() => setIndex(i => Math.max(0, i - 1))} disabled={index === 0}
              style={{width: '44px', height: '44px', borderRadius: '50%', background: index === 0 ? 'rgba(255,255,255,0.05)' : 'rgba(255,255,255,0.1)', border: '1px solid rgba(255,255,255,0.15)', color: index === 0 ? 'rgba(255,255,255,0.2)' : '#f5ebe0', fontSize: '16px', cursor: index === 0 ? 'default' : 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', transition: 'all 0.2s ease'}}>←</button>
            <button onClick={() => setIndex(i => Math.min(produits.length - 1, i + 1))} disabled={index === produits.length - 1}
              style={{width: '44px', height: '44px', borderRadius: '50%', background: index === produits.length - 1 ? 'rgba(255,255,255,0.05)' : '#8b1a1a', border: '1px solid rgba(255,255,255,0.15)', color: index === produits.length - 1 ? 'rgba(255,255,255,0.2)' : '#fff', fontSize: '16px', cursor: index === produits.length - 1 ? 'default' : 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', transition: 'all 0.2s ease'}}>→</button>
          </div>
        </div>
